...
import { ChannelType } from "npm:discord-api-types";
...
export async function fetchAllMembersFromGuild(
  ...
  while (true) {
    ...
    const fetchedMembers = await guild.members.fetch(options) as Collection<
      Snowflake,
      GuildMember
    >;
...
export async function fetchAllMessagesFromGuild(
  ...
  const textChannels = guild.channels.cache.filter(
    (channel: { type: Channel }) => channel.type === ChannelType.GuildText,
  ) as Map<Snowflake, TextChannel>;
