...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        const aggregatedMessages = [
            ...
            .map(msg => `${msg.author.username}: ${msg.content} [${msg.createdAt.toISOString()}]`)
            .sort((a, b) => new Date(a.split('[')[1]).getTime() - new Date(b.split('[')[1]).getTime()) // Sort by timestamp
            .join('\n'); // Join the messages into a single string

        // Step 6: Analyze Sentiment Using the Agent
        console.log('Analyzing Sentiment...');
        const analysisResult = await analyzeSentimentWithAgent(aggregatedMessages); // Pass the formatted conversation
...
): Promise<{ guild: Guild, user: GuildMember, days: number | undefined } | string> {
