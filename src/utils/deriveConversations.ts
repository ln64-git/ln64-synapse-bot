...
import {Conversation, FiresideMessage} from "../types.ts"
...
export async function deriveConversations(
  ...
  for (const message of sortedMessages) {
    ...
    if (
      ...
      for (const attachment of message.attachments) {
        if (attachment.url && isImageUrl(attachment.url)) {
          ...
          const summaryPrompt = `Provide a concise summary for the following image URL: ${attachment.url}`
          try {
    ...
    if (message.messageContent && message.messageContent.trim() !== "") {
      ...
      const combinedText =
        message.attachments.map((att) => att.summary || "").join(" ") +
        " " +
