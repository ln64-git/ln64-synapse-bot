...
import { RESTPostAPIApplicationCommandsJSONBody } from 'discord-api-types/v10'; // Import the type
...
async function registerCommands() {
  ...
  // Define the type of commands array
...
for (const file of commandFiles) {
  const command = require(`./commands/${file}`);
  (client as any).commands.set(command.data.name, command); // Make sure to set the command using its name
}
